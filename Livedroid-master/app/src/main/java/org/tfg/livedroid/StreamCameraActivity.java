        ...
        implements SurfaceHolder.Callback
{
    ...
    private static final String WAKE_LOCK_TAG = "LiveDroid";

    //MY ADDS PMA
    private DroidServer webServer = null;

    //END ADDS

    private boolean scan_active = false;
    ...
    public boolean onOptionsItemSelected(final MenuItem item)
    {
        ...
        startActivity(new Intent(this, LiveDroidPreferenceActivity.class));
        return true;
    ...
    public void startStreaming(View view) {
        ...
        new AlertDialog.Builder(this)
                .setTitle("Stream")
                .setMessage("Comenzando el Stream!")
    ...
    public void enableHTTPServer(View view) {

        Button starthttp = (Button)findViewById(R.id.enablehttp);
        Button stophttp = (Button)findViewById(R.id.disablehttp);

        Button udpButton = (Button)findViewById(R.id.disableUDP);

        String haveIp = tryGetIpV4Address();

        new AlertDialog.Builder(this)
                .setTitle("HTTP Server")
                .setMessage("HTTP Server Started on http://"+ haveIp +":8081")
                .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        // continue with delete
                    }
        ...
        if(haveIp != null){
            ...
            try{
                ...
                webServer = new DroidServer(8081, this);
                udpButton.setVisibility(View.VISIBLE);

            } catch(IOException e){ webServer = null; }

        }

        starthttp.setVisibility(Button.INVISIBLE);
        stophttp.setVisibility(Button.VISIBLE);

    }
    public void DisableHTTPServer(View view) {
        ...
        new AlertDialog.Builder(this)
                .setTitle("HTTP Server")
                .setMessage("HTTP Server Stopped")
                .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        // continue with delete
                    }
        ...
        starthttp.setVisibility(Button.VISIBLE);
        stophttp.setVisibility(Button.INVISIBLE);
        udpButton.setVisibility(View.INVISIBLE);

    }

    public void viewSettings(View view){

        Intent i = new Intent(getApplicationContext(),LiveDroidPreferenceActivity.class);
        startActivity(i);
    ...
    public void stopListening(){
        ...
        Context MJpegCont = mCameraStreamer.MJpegContext();

    }

    public void UDPFinder(View view){
        ...
        if(webServer != null) {
           ...
           if( webServer.getUDPStat() ){

               webServer.stopUDP();

               udpButton.setText("Start UDP");
               int blueToStart = Color.parseColor("#58ACFA");
               udpButton.setBackgroundColor(blueToStart);
               Log.d(TAG, "UDP listening Stopped");
           }
           else{

               webServer.startUDP();

               udpButton.setText("Stop UDP");
               int redToStop = Color.parseColor("#ff4d4d");
               udpButton.setBackgroundColor(redToStop);
               Log.d(TAG, "UDP listening Started");
           }
       }
